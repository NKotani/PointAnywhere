...
testMODE = int(sys.argv[2]) # 1のとき余計な画像保存しない、テスト済みのをスキップする
skelton_path = sys.argv[3] # 'skelton/'

save_dir = increment_path(Path('./Experiment') / 'jp_great_op', exist_ok=False)  # increment run
print(save_dir)
...
for file in files:
    ...
    people = {}
    name_txt = os.path.join(skelton_path, sphere+ '/labels/' + sphere + '.txt') # 'skelton/'+sphere+'/labels/'
    try:
    ...
    equ = E2P.Equirectangular(file) # Load equirectangular image
    if  testMODE ==  0: # 定性評価のとき
        equ.GetPerspective(fov, theta, phi, pers_h, pers_w, file, outputOmni_f) # imgがreturnされるけど使わない
    ...
    # Test.write_to_file(f'{fov} {theta} {phi} {pers_h} {pers_w}', human_filename+'.txt')
    # np.savez(human_filename, candidate=candidate, subset=subset)
    ...
    if joints_num == [-1]:
        ...
        # vec = PointingVector.get_vec(joints, joints_num, theta, 0, pers_w, pers_h, fov)
    ...
    if vec[0] == 0: # 周期60度
        ...
        equ.GetPerspective(60, rot_z[0], rot_y[0], 640, 640, file, os.path.join(outDirect_f,sphere+'/'))
        for i in [1,2,3,4]:
            index = (start_num + i * 30) % 1080 # 10度ずつ進む
            rot_z.append(int(z[index]))
            rot_y.append(int(y[index]))
            equ.GetPerspective(60, rot_z[i], rot_y[i], 640, 640, file, os.path.join(outDirect_f,sphere+'/'))
        ...
        for i in range(9): # 上端と下端に到達すると30°進まないので幅を持たせておく
            if i == 0:
                ...
                if  opposite >= 180: # 指差している起点から左右180°反対まで
                    ...
                    break
                # elif vec[1] < 0 and rot_y[i-1] > wrist[1]: # 下向に進んでいてスタート地点を追い越した(U)
                #     print('下向に進んでいてスタート地点を追い越した')
                #     Test.write_stdout('下向に進んでいてスタート地点を追い越した', stdout, testResult_f)
                #     break
                # elif vec[1] > 0 and rot_y[i-1] < wrist[1]: # 上向に進んでいてスタート地点を追い越した(^)
                #     print('上向に進んでいてスタート地点を追い越した')
                #     Test.write_stdout('上向に進んでいてスタート地点を追い越した', stdout, testResult_f)
                    # break
    ...
    for i in range(len(rot_z)):
        ...
        # logging.info('return code: {}'.format(proc.returncode))
    ...
    # Test.write_obj(joints_num, vec, obj, sphere, testResult_f)
    ...
    ground_truth = '../dataset/ROI/' + sphere + '.txt'
